	...
	public Dictionary getDictionaryFromGeneratedClass(InputStream inputStream) {
		...
		try {
			...
			if (saxParseException != null) {
				...
				String strError = String.format("%4$sThis XML stream structure could not be unmarshalled due to SAXParser error %4$s[%1$s].%4$sError was found on the line [%2$d], column [%3$d].%4$s",
				...
				throw new EPSCommonException("This XML stream structure could not be unmarshalled due to SAXParser error.", jabEx);
			}

		} catch (Exception e) {
			throw new EPSCommonException("This XML stream structure could not be unmarshalled.", e);
		} finally {
